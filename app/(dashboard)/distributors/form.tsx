...
import { createDistributor, updateDistributor, getDistributor } from '@/lib/api/distributors'
...
export default function DistributorForm({ id, onSuccess, onCancel }: DistributorFormProps) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Omit<Distributor, 'id' | 'created_at'>>({
    name: '',
    phone: '',
    fax: '',
    tax_id: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        // 更新經銷商
        const updated = await updateDistributor(id, form)
        if (updated) {
        ...
        // 創建新經銷商
        const created = await createDistributor(form)
        if (created) {
          if (onSuccess) onSuccess()
          else router.push('/distributors')
        } else {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-xl font-semibold mb-6">{id ? '編輯經銷商' : '新增經銷商'}</h2>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="name">
              經銷商名稱 <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              id="name"
        ...
        <div className="flex justify-end space-x-2">
          <button
            type="button"
            onClick={onCancel || (() => router.push('/distributors'))}
